...
def concatenate_attention_weights(param: torch.Tensor, n_embd: int) -> torch.Tensor:
    # Assuming param has shape [1024, 512] and n_embd is 512
    # Split into 3 parts along the first dimension and then concatenate along the second dimension
    split_size = param.shape[0] // 3
    assert split_size * 3 == param.shape[0], "Input tensor's first dimension is not divisible by 3"
    
    q = param[:split_size, :]
    k = param[split_size:2*split_size, :]
    v = param[2*split_size:, :]
