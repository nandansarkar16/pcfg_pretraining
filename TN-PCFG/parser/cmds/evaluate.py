...
class Evaluate(CMD):
    ...
    def __call__(self, args, eval_dep=False, decode_type='mbr'):
        ...
        if not eval_dep:
            likelihood = self.evaluate(test_loader_autodevice, eval_dep=eval_dep, decode_type=decode_type)
        else:
            metric_uas, likelihood = self.evaluate(test_loader_autodevice, eval_dep=eval_dep, decode_type=decode_type)
            print(metric_uas)
        #print(metric_f1)
        print(likelihood)
